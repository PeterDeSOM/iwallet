...
import { 
    ...
    ISom32Account, ISom32Extenal, ISom32PsbtDecoded, ISOM32RawTxToSend, ISOM32TxFormToSend, ISom32AccountShared, ISom32AccountSingle 
...
export class WalletService {
    ...
    public addressValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
        ...
        if(address.search(/[\W_]+/g) >= 0) return null
    ...
    public createWallet(
        ...
        // NOTE: 
        // 1. wallet service's network type was already set by app init.
        // 2. bip32 is used only for single account, so that salt is needed for 
        //    creating a single account, not for the shared account.
        // 3. it assumes that a single account unconditionally has to be created.
                    ...
                    desc    : "sh(wsh(multi(" + m + "," + (
                        pubkeys.map(
                            k => {
                                ...
                                // TO DO: validate address type, and enter right address type
    ...
    public createPsbt(rawtx: ISOM32RawTxToSend, txhexs: { txid: string, hex: string }[]): string {
        ...
        // TO DO: for bitcoin network compatible, add user interactive function that converts hex string of 
    ...
    public updatePsbt(psbt: string): string {
        ...
        // TO DO: validate this psbt's multisig address match with currently opened multisig address
...
class Som32 {
    ...
    public getname(address: string): string {
        let single = !!this._account.single ? this._account.single.find(s => s.address == address) : null
        let shared = !!this._account.shared ? this._account.shared.find(s => s.address == address) : null
    ...
    public add(wallet: any): ISom32Extenal {
        ...
        // TO DO: validate wallet's version, network and ect necessaries ... 
    ...
    public import(wallet: any) {
        // NOTE: network was already set from the wallet service on, so, at this point, it 
        // doesn't need to set network.
        // this._network = (
        //     wallet.network == C.NETWORK_MAINNET ? 
    ...
    public forQR(type: 'wallet' | 'single' | 'shared' | 'pubkey', address?: string): string {
        ...
        switch(type) {
            ...
            case C.ACCOUNT_TYPE_SINGLE:
                let single = this._account.single.find(s => s.address == address)
                som32 = {
                    version : C.APP_VERTION,
                    network : network,
                    account : {
            ...
            case C.ACCOUNT_TYPE_SHARED:
                let shared = this._account.shared.find(s => s.address == address)
                som32 = {
                    version : C.APP_VERTION,
                    network : network,
                    account : {
...
class Bip32 {
    ...
    public m(seed: Buffer, path?: string): IBitcoinjsBIP32 {
         ...
         // TO DO: check m's deriving path is right
