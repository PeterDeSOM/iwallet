...
import { 
    Alert, AlertInvalidFileCode, AlertFileNotFound, AlertInvalidPassword, AlertInvalidWalletImportFormat, 
    AlertNotMatchedNetwork, AlerNotMatchedMultiSigWithSingle, DialogPassphraseService, Som32ConfigService 
} from '@som32/services';
import { IBlockcypherAddressBalance, ISom32Extenal, ISom32AccountShared, ISom32AccountSingle } from '@som32/interfaces'
...
export class HomeComponent implements OnInit, OnDestroy {
     ...
     */
    ngOnInit(): void {
        this.tabgroup._handleClick = this._interceptTabChange.bind(this);
        this._apiService.getBalance(this.address).subscribe(
            _ => {
                ...
                this._balancePieData = [
                    parseFloat((this.blockcypherAddressBalance.total_received / this.blockcypherAddressBalance.balance * 100).toFixed(2)),
                    parseFloat((this.blockcypherAddressBalance.total_sent / this.blockcypherAddressBalance.balance * 100).toFixed(2)),
                    this._satoshiToBitcoin.transform(this.blockcypherAddressBalance.total_received),
                    this._satoshiToBitcoin.transform(this.blockcypherAddressBalance.total_sent)
                ];
            }, err => {
                console.log(err);
            }
    ...
    public import(event) {
        ...
        reader.onload = () => {
            ...
            // TO DO: eliminate new line and etc, not JSON compatible, characters
